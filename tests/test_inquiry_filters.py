...
def test_approval_print_highlights_selected_quote_with_yellow_background(client, test_db):
    ...
    assert response.status_code == 200
    html = response.data.decode("utf-8")
    assert ".quote-selected-row { background: #fff3cd;" in html
    assert '<tr class="quote-selected-row">' in html
...
def test_gx_inquiry_creates_gx_material_for_cross_region_item(client, test_db):
    ...
    cursor.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (item_id, None, 12, 11.88, 0.01, 24, 1, 1, "2026-06-06 10:00:00"),
    )
    test_db.commit()
...
def test_export_draft_inquiry_xlsx_matches_quote_template(client, test_db):
    ...
    assert [sheet.cell(3, col).value for col in range(1, 11)] == [
        "序号", "材料名称", "规格型号", "品牌", "是否国标", "单位", "数量", "库内价", "测试供应商单价1%专票", "测试供应商总价"
    ]
    assert [sheet.cell(4, col).value for col in range(1, 9)] == [
        1, "测试材料", "加厚", "测试品牌", "是", "个", 5, 12.34
    ]
    assert sheet["J4"].value == '=IF(I4="","",I4*$G4)'
