...
def test_tanxiang_can_use_supplier_create_button():
    app_source = Path("static/js/app.js").read_text(encoding="utf-8")
    index_source = Path("index.html").read_text(encoding="utf-8")

    assert "supplier-create-btn" in index_source
    assert "canCreateSupplier" in app_source
    assert "tanxiang" in app_source
    assert "谭香" in app_source
