...
def generate_order_no():
    ...
    cursor.execute("SELECT COUNT(*) FROM purchase_orders WHERE order_no LIKE ?", (f'CGDD-{today}%',))
    count = cursor.fetchone()[0] + 1
    conn.close()
    return f'CGDD-{today}-{str(count).zfill(3)}'
...
@order_bp.route('/purchase-orders/<int:order_id>', methods=['GET'])
def get_order(order_id):
    ...
    cursor.execute("""
        SELECT pod.*, m.material_name, m.specification, m.material_code, u.unit_name
    ...
    return jsonify({'success': True, 'data': order, 'details': details})


@order_bp.route('/purchase-orders', methods=['POST'])
def create_order():
    """创建采购订单"""
    data = request.json
    ...
    try:
        ...
        """, (
            order_no, data.get('order_type', '集采'), data.get('project_id'),
            data.get('supplier_id'), total_amount, user['id'], '待审批',
            '待入库', now, data.get('remark', '')
        ))
        ...
        for d in details:
            ...
            cursor.execute("""
                INSERT INTO purchase_order_details (order_id, material_id, quantity, unit_price, amount, in_quantity)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (order_id, d.get('material_id'), d.get('quantity', 0),
                  d.get('unit_price', 0), amount, 0))
...
@order_bp.route('/purchase-orders/<int:order_id>/approve', methods=['POST'])
def approve_order(order_id):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, ('purchase_order', order_id, user['id'], user['real_name'], result_text, remark, now))
...
@order_bp.route('/purchase-orders/<int:order_id>/approval-history', methods=['GET'])
def get_order_approval_history(order_id):
    """获取采购订单审批历史"""
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("""
        ...
        WHERE ar.order_type = 'purchase_order' AND ar.order_id = ?
...
@order_bp.route('/purchase-orders/<int:order_id>/approval-print', methods=['GET'])
def print_order_approval(order_id):
    """打印采购订单审批签字单"""
    conn = get_db()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT po.*, u.real_name as applicant_name, s.supplier_name, p.project_name
        FROM purchase_orders po
        LEFT JOIN users u ON po.applicant_id = u.id
        LEFT JOIN suppliers s ON po.supplier_id = s.id
        LEFT JOIN projects p ON po.project_id = p.id
    ...
    amount_chinese = amount_to_chinese(order['total_amount'])
    ...
    <head>
        <meta charset="UTF-8">
        <title>采购订单审批签字单 - {order['order_no']}</title>
        <style>
    ...
    <body>
        <div class="print-page">
            <div class="print-header">
                <h1>采购订单审批签字单</h1>
                <div>{escape(order['order_no'])}</div>
            </div>

            <div class="info-row">
                <div class="info-item"><label>订单类型：</label>{escape(order.get('order_type', '-'))}</div>
                <div class="info-item"><label>供应商：</label>{escape(order.get('supplier_name', '-'))}</div>
            </div>
            <div class="info-row">
                <div class="info-item"><label>项目：</label>{escape(order.get('project_name', '-'))}</div>
                <div class="info-item"><label>申请人：</label>{escape(order.get('applicant_name', '-'))}</div>
            </div>
            <div class="info-row">
                <div class="info-item"><label>当前状态：</label><strong>{escape(order.get('approval_status', '-'))}</strong></div>
                <div class="info-item"><label>采购状态：</label>{escape(order.get('purchase_status', '-'))}</div>
            </div>
...
"""
    ...
    html += f"""                </tbody>
            ...
            <div style="text-align: right; margin: 15px 0;">
                <strong>总金额：¥{order.get('total_amount', 0):.2f}</strong>
...
"""
            ...
            if order['approval_status'] == level_info['status'] or (order['approval_status'] == '待审批' and level_info['level'] == '材料员'):
                html += f"""
                        <tr>
                            <td>{level_info['level']}</td>
...
"""
    ...
    html += f"""                    </tbody>
            ...
            <div style="margin-top: 30px; font-size: 12px; color: #666;">
                <strong>备注：</strong>{escape(order.get('remark') or '无')}
            </div>
